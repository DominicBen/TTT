...
function GM:HUDWeaponPickedUp(wep)
   ...
   local role      = LocalPlayer().GetRoleClass and LocalPlayer():GetRoleClass() or Unknown
   pickup.color    = role:getColor()
...
function GM:HUDDrawPickupHistory()
   ...
   for k, v in pairs(self.PickupHistory) do
      if v.time < CurTime() then
         ...
         local rx, ry, rw, rh = math.Round(v.x - 4), math.Round(v.y - (v.height / 2) - 4),
             math.Round(self.PickupHistoryWide + 9), math.Round(v.height + 8)
         local bordersize = 8
         ...
         if v.amount then
            draw.SimpleText(v.amount, v.font, v.x + self.PickupHistoryWide + 2, v.y - (v.height / 2) + 2,
               Color(0, 0, 0, alpha * 0.75), TEXT_ALIGN_RIGHT)
            draw.SimpleText(v.amount, v.font, v.x + self.PickupHistoryWide, v.y - (v.height / 2),
               Color(255, 255, 255, alpha), TEXT_ALIGN_RIGHT)
         end
