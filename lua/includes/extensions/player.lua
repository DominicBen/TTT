...
function meta:SetPData(name, value)
	...
	local key = Format("%s[%s]", self:SteamID64(), name)
	return sql.Query("REPLACE INTO playerpdata ( infoid, value ) VALUES ( " ..
		SQLStr(key) .. ", " .. SQLStr(value) .. " )") ~= false
end
...
--- @return fun() -- Returns a function that can be used to iterate over PlayerCache
--- @return Player[] -- The PlayerCache table itself
--- @return number -- The starting index for iteration
